pro charis_subsklip,pfname,gemini=gemini,nomask=nomask,prefname=prefname,nfwhm=nfwhm,drsub=drsub,$
...
savecoeff=savecoeff,$
usecoeff=usecoeff,$
rmin=rmin,rmax=rmax,fc=fc,angoffset=angoffset,outfile=outfile,$
...
print,'charis_subsklip.pro: PCA/KLIP PSF subtraction method, Version 1.1'
print,'Written by T. Currie (2014), adapted for CHARIS IFS Data (4/2017)'
print,''
print,'**Calling Sequence**'
print,'charis_subsklip,pfname,prefname=prefname,nfwhm=nfwhm,npca=npca,rmin=rmin,rmax=rmax,drsub=drsub,outfile=outfile'
print,''
print,'Example:'
...
print,''
print,'Limitations'
print,' - currently cannot do forward-modeling (yet; e.g. no saves/importing of coefficients) and thus no usage of fc switch'
...
if keyword_set(savecoeff) then openw,1,'locicoeff.dat'
...
;Frame Selection
if ~keyword_set(nref) then nref=1000.
...
nf_use3=nf_use2[coeffstouse]
    ...
    iaim_2load=intersect(iaim,intersect(iaim,heiaim_loaded,/xor_flag))
...
for nf=0,nfiles-1 do begin
    ...
    ;help,dim,tmpdir,filestmp[nf],ind
...
;it is called loci, not klip in suffix even though reduction is klip
...
;gpi_northup,im,h0,h1
h0_col=h0
h1_col=h1
;gpi_northup,im_collapsed,h0_col,h1_col
