...
if keyword_set(noreprocess) then begin
   ...
   for i=0, n_elements(result)-1 do begin ;trim off directory and suffix info
      result[i] = repstr(result[i], '_inputcolsc', '')
      result[i] = strmid(result[i], max(strsplit(result[i], '/')), strlen(result[i]))
      
      loc = where(mncopy eq result[i]) ; has the file already been analyzed
      if loc ne -1 then begin
         print, "skipping", modelname[loc]
         print
         remove, loc, modelname, mncopy
      endif
   endfor
endif
...
if keyword_set(calcattenfact) then begin 
   ;the data cube
    ...
    ;here, we aren't defining chisq in a robust way.  Leave that to
    ;the collapsed cube for now. 
        ;old method
        ...
        ;new method
        synthcube_scale[icube]=total((refcube_con*imcube_con/sigdata^2)[roitotal],/nan)/total((imcube_con*imcube_con/sigdata^2)[roitotal],/nan)
...
if keyword_set(noreprocess) then begin
   ...
   print, "check before:"
   spawn, 'ls reduc/proc/model_in/*fits | wc -l'
   spawn, 'mv reduc/proc/model_in/*fits /home/sgoebel/thayne/pipeline/charisred/reduc/proc/model_in/'
   spawn, 'mv reduc/proc/model_psfsub/*fits /home/sgoebel/thayne/pipeline/charisred/reduc/proc/model_psfsub/'
   print, "check after"
   spawn, 'ls reduc/proc/model_in/*fits | wc -l'
   wait, 3
